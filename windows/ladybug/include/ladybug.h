...
typedef struct LadybugCameraInfo
{
   ...
   unsigned int firmwareMajor; /** Firmware major version. */
   unsigned int firmwareMinor; /** Firmware minor version. */
   unsigned int firmwareBuildType; /** Firmware build type. */
   unsigned int firmwareBuildNum; /** Firmware build number. */
...
typedef struct LadybugToneMappingParams
{
   ...
   LadybugToneMappingParams()
   {
       ...
       memset(reserved, 0x0, 27 * sizeof(unsigned int));
   }
...
/**
 * Gets the maximum JPEG compression quality setting on the camera.
 * Only valid if ladybugGetAutoJPEGQualityControlFlag() is true.
 *
 * @param context   - The LadybugContext to access.
 * @param piQuality - The maximum JPEG quality that the camera will use.
 *
 * @return A LadybugError indicating the success of the function.
 *
 * @see ladybugSetJPEGQuality()
 */
LADYBUGDLL_API LadybugError 
ladybugGetAutoJPEGMaxQuality(
    LadybugContext context, 
    int* piQuality);

/**
 * Sets the maximum JPEG compression quality setting on the camera.
 * Only valid if ladybugGetAutoJPEGQualityControlFlag() is true.
 *
 * @param context  - The LadybugContext to access.
 * @param iQuality - The maximum JPEG quality that the camera will use,
...
/**
 * Checks the validity of a ladybugImage that is in a JPEG compressed data format.
 *
 * @param context        - The LadybugContext to access.
 * @param pImage         - A pointer to the JPEG image to be checked.
 * @param pIsValid       - A pointer to a boolean to return the validity
 *                         of the image.
 *
 * @return A LadybugError indicating the success of the function.
 *

 ...
 * 
 *  Please see the "Stream File Format" section in the Ladybug Technical 
 *  Reference for additional information about Ladybug stream files.
 *
 */
 ...
 *
 * At a high level, the stream file data sequence is detailed below:
 ...
 * please see the "Stream File Format" section in the Ladybug Technical Reference.
 */
typedef struct LadybugStreamHeadInfo
{
   ...
   /** 
    * Base unit serial number for the Ladybug2 camera or earlier models.
    * For all newer cameras, it is equivalent to serialHead. 
    */
   ...
   /** The size of the calibration data for the camera, in bytes. */
   unsigned int ulConfigurationDataSize;
 
   /** 
    * The number of images in the current stream file segment.
    * This is not equal to the total number of images in the entire stream file.
    */
   ...
   unsigned int firmwareBuildType;
   unsigned int firmwareBuildNum;

   /** Reserved. */
   unsigned char reservedSpace[508];

   /** 
    * Image offset index table. The offset value is relative to the beginning
    * of the stream file.
    *

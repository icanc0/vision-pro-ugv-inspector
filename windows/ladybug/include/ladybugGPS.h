//=============================================================================
// Copyright (c) 2011-2023 FLIR Systems, Inc. All Rights Reserved.
//
...
/**
 * A data structure for configuring the GPS NMEA / PPS time synchronization
 * feature that is available on LD5+ and LD6 cameras.
 */
typedef struct _GpsTimeSyncSettings
{
   bool enablePps;         /**< Whether the camera should accept PPS time pulses. */
   bool enableGpsTimeSync; /**< Whether to enable NMEA time synchronization. */
   /**
    * Polarity of PPS signal for camera to latch to.
    * A value of false (low to high) is the default value on the camera
    * unless configured otherwise.
    * 
    * True = falling edge (high to low)
    * False = rising edge (low to high). 
    */
   bool ppsPolarity;
   unsigned int baudRate; /**< Baud rate of incoming NMEA data. */
} GpsTimeSyncSettings;

/**
* A data structure to obtain feature availability for the 
* GPS NMEA / PPS time syncrhonization feature.
*/
typedef struct _GpsTimeSyncInfo
{
    bool isPpsSupported; /**< GPS NMEA / PPS time synchronization support. */
    bool isPpsPolaritySupported; /**< PPS polarity configuration support. */
} GpsTimeSyncInfo;
...
/**
 * @defgroup GPSTimeSynchronizationMethods GPS Time Synchronization Methods
 *
 * This group of functions is used to configure the GPS / PPS
 * time synchronization feature that is available on Ladybug5+ and Ladybug6
 * cameras.
 */
...
/**
 * Sets the GPS time synchronization feature. Note that these functions
 * take in a LadybugContext instead of a LadybugGPSContext.
 *
...
LADYBUGDLL_API LadybugError 
ladybugGetGpsTimeSyncInfo(LadybugContext context, GpsTimeSyncInfo& timeSyncInfo);
